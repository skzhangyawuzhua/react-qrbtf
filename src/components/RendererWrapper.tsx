...
export function drawIcon({ qrcode, title, titleSize, titleColor, titleAlign, icon, iconScale = .33, styles }: RendererProps) {
    ...
    if (icon) {
        pointList.push(<image key={2} xlinkHref={icon} width={iconSize} height={iconSize} x={iconXY} y={iconXY} />);
    }
    ...
    if (title) {
        ...
        pointList.push(<text key={3} x={nCount / 2} y={fontY} fill={titleFontColor} style={{ ...titleStyle , fontSize:titleFontSize+'px' }} font-size={titleFontSize+'px'} textAnchor="middle">{title}</text>)
    }
